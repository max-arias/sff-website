...
export function caseInsert(runId: string, slug: string, part: GenericPart, casePart?: CasePart) {
  ...
  return `insert into cases (${[
    ...
    escapeSql(cd?.cpuCoolerHeightMm ?? dim(part, ["cpu_cooler_height"])),
    escapeSql(cd?.gpuLengthMm ?? dim(part, ["gpu_length"])),
    escapeSql(cd?.gpuWidthMm ?? dim(part, ["gpu_width"])),
    escapeSql(cd?.gpuThicknessMm ?? dim(part, ["gpu_thickness", "thickness"])),
    escapeSql(cd?.pcieSlots ?? dim(part, ["pcie_slots"])),

...
test("part list text carries the build URL and one line per selected slot", async () => {
  const store = new InMemoryCatalogStore({
    cases: [fakeCase({ id: "c", name: "Test Case One" })],
    gpus: [fakeGpu({ id: "g" })],
    parts: [],
  });
  const url = new URL("http://localhost/build?case=c&gpu=g&kind=psu");
  const view = await getBuildView(url, store);

  const lines = view.partListText.split("\n");
  assert.equal(lines[0], `SFF Builder Part List: ${url.href}`);
  assert.deepEqual(lines.slice(2, -2), [
    "Case: TestCo Test Case One",
    "GPU: NVIDIA RTX 4090 GeForce RTX 4090",
  ]);
  assert.equal(lines.at(-1), "Generated by SFF Builder");
  assert.equal(view.partListText, view.partListText.trimEnd());
});

test("part list text skips unselected slots", async () => {
  const store = new InMemoryCatalogStore({
    cases: [fakeCase({ id: "c" })],
    gpus: [fakeGpu({ id: "g" })],
    parts: [],
  ...
  const view = await getBuildView(
    new URL("http://localhost/build?case=c&kind=case"),
    store,
  );

  const partLines = view.partListText
    .split("\n")
    .filter((line) => line.includes(": ") && !line.startsWith("SFF Builder"));
